...
| `CODEX_TURN_STATE_REFINE_PARALLEL` | 否 | `8` | 死磕刷新（`codex_turn_state_refine_enabled`）单轮并发探测数（1-8）。轮换代理池下 N 并发等于一轮同时试 N 个出口 IP，命中 292 即取消其余在途探测；`1` 退回串行形态。额度总消耗与串行相当（都是打到命中为止），仅缩短耗时 |
