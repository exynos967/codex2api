...
const (
	...
	// 行数 × 列数必须 < postgresMaxBindParams（有测试锁这条不变式）；
	// 加列时同步下调。65535/67 ≈ 978，留余量取 950。
	maxUsageLogInsertRowsPerSQL = 950
...
type UsageLog struct {
	...
	// UpstreamResponseModel 是上游自报的实际出活模型（openai-model 响应头/WS
	// metadata 帧）；UpstreamModelMismatch 三态：nil=未观测，true=与发往上游的
	// 模型不一致。用于审计上游静默换模型/降级路由。
...
func (db *DB) InsertUsageLog(ctx context.Context, log *UsageLogInput) error {
	...
	db.logBuf = append(db.logBuf, usageLogEntry{
		...
		UpstreamResponseModel:  clampUsageLogText(log.UpstreamResponseModel, usageLogTextMaxLen),
...
type UsageLogInput struct {
	...
	InjectedTurnState string
	UpstreamTurnState string
	// UpstreamResponseModel / UpstreamModelMismatch 见 UsageLog 同名字段。
	UpstreamResponseModel string
	UpstreamModelMismatch *bool
	AccountID             int64
	...
	if len(logsToStore) > 0 {
		stmt, err := tx.PrepareContext(ctx,
			`INSERT INTO usage_logs (account_id, credential_generation, channel, client_ip, endpoint, model, effective_model, prompt_tokens, completion_tokens, total_tokens, status_code, duration_ms,
				  ...
				  client_user_agent, upstream_user_agent, user_agent_overridden, internal_reason, parent_request_id, prompt_policy_incident_id, request_id, upstream_request_id, upstream_proxy_id, upstream_proxy_name, injected_turn_state, upstream_turn_state, user_billing_mode, image_unit_price, billed_image_count, upstream_response_model, upstream_model_mismatch)
...
func (db *DB) batchInsertLogsChunk(ctx context.Context, execer sqlExecer, batch []usageLogEntry) error {
	...
	for _, e := range batch {
		...
		valueArgs = append(valueArgs, e.AccountID, e.CredentialGeneration, e.Channel, e.ClientIP, e.Endpoint, e.Model, e.EffectiveModel, e.PromptTokens, e.CompletionTokens, e.TotalTokens, e.StatusCode, e.DurationMs,
			...
			e.APIKeyID, e.APIKeyName, e.APIKeyMasked, e.ImageCount, e.ImageWidth, e.ImageHeight, e.ImageBytes, e.ImageFormat, e.ImageSize, e.AccountBilled, e.UserBilled,
			e.IsRetryAttempt, e.AttemptIndex, e.UpstreamErrorKind, e.ErrorMessage, e.ViaWebsocket,
			e.ClientUserAgent, e.UpstreamUserAgent, e.UserAgentOverridden, e.InternalReason, e.ParentRequestID, nullablePromptPolicyIncidentID(e.PromptPolicyIncidentID), e.RequestID, e.UpstreamRequestID, e.UpstreamProxyID, e.UpstreamProxyName, e.InjectedTurnState, e.UpstreamTurnState, e.UserBillingMode, e.ImageUnitPrice, e.BilledImageCount, e.UpstreamResponseModel, e.UpstreamModelMismatch)
...
// nullableBoolValue 把可空的 BOOLEAN 扫描结果转成三态指针（NULL → nil=未观测）。
...
func (db *DB) ListRecentUsageLogs(ctx context.Context, limit int) ([]*UsageLog, error) {
	...
	for rows.Next() {
		...
		var upstreamMismatch sql.NullBool
		var createdAtRaw interface{}
		if err := rows.Scan(&l.ID, &l.AccountID, &l.ClientIP, &l.Endpoint, &l.Model, &l.EffectiveModel, &l.PromptTokens, &l.CompletionTokens, &l.TotalTokens, &l.StatusCode, &l.DurationMs,
			...
			&l.InternalReason, &l.ParentRequestID, &l.PromptPolicyIncidentID, &l.RequestID, &l.UpstreamRequestID, &l.UpstreamProxyID, &l.UpstreamProxyName, &l.InjectedTurnState, &l.UpstreamTurnState, &l.UpstreamResponseModel, &upstreamMismatch,
			&credentialRaw, &l.AccountName, &createdAtRaw); err != nil {
			return nil, err
		}
		l.AccountEmail = accountEmailFromRawCredentials(credentialRaw)
		l.UpstreamModelMismatch = nullableBoolValue(upstreamMismatch)
...
func (db *DB) ListUsageLogsByTimeRangePaged(ctx context.Context, f UsageLogFilter) (*UsageLogPage, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&l.ID, &l.AccountID, &l.ClientIP, &l.Endpoint, &l.Model, &l.EffectiveModel, &l.PromptTokens, &l.CompletionTokens, &l.TotalTokens, &l.StatusCode, &l.DurationMs,
			...
			&l.ClientUserAgent, &l.UpstreamUserAgent, &l.UserAgentOverridden, &l.Channel, &l.InternalReason, &l.ParentRequestID, &l.PromptPolicyIncidentID, &l.RequestID, &l.UpstreamRequestID, &l.UpstreamProxyID, &l.UpstreamProxyName, &l.InjectedTurnState, &l.UpstreamTurnState, &l.UpstreamResponseModel, &upstreamMismatch,
